...
if($_SESSION['uid'] > 0) {
   ...
   try
   {
      ...
      {
         $entry = $row['content'];
         echo "<li><button name=\"triggerdelete\" onclick=\"Modalbox.show('Are you sure to delete this item?<br> <button type=\"submit\" name="delete" value=\'$entry\'>Yes</button> <button type=\"button\" onclick=\'Modalbox.hide()\'>No</button>', {title: this.title, width: 300});\">DELETE</button> $entry</li>";
      }
      $stmt -> closeCursor();
   }
